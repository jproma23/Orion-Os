...
async def principal() -> None:
    ...
    # Escuta desligavel (2026-07-20): sem escuta, nada de mic/Whisper/VAD -
    # os dois modelos nem sao carregados (RAM do Notebook e o recurso mais
    # disputado, EDR-0019). A conversa entra por texto (chat da pagina
    # /controle) e a resposta continua saindo por TTS + tela.
    escuta_habilitada = secao_voz.get("escuta_habilitada", True)

    indice_mic = None
    transcritor = None
    transcritor_ativacao = None
    if escuta_habilitada:
        ...
        logger.info("Escuta por microfone DESLIGADA (voice.escuta_habilitada) - modo chat+TTS")
    ...
    # Chat de texto: a resposta e gerada aqui (mesmo Mission Planner da voz)
    # e precisa CHEGAR ao Pi para aparecer na dashboard.
    bus.subscribe("chat.resposta_robo", _repassar_ao_pi)
    ...
    # Chat de texto da pagina /controle: mesmo cerebro da voz, sem passar
    # pelo mic. O texto chega do Pi (chat.mensagem_usuario), e processado
    # pelo mesmo Mission Planner e a resposta volta por chat.resposta_robo
    # -> Pi -> tela, ALEM de ser falada (TTS): o dono escreve, o Fofao
    # responde por audio e mensagem.
    async def _ao_receber_chat(evento) -> None:
        texto = str(evento.dados.get("texto", "")).strip()
        if not texto:
            return
        logger.info("Chat recebeu: %s", texto)
        resposta = await processar_comando(texto)
        # a tela primeiro (rapido), a fala depois (bloqueia segundos)
        await bus.publish("chat.resposta_robo", {"texto": resposta})
        try:
            await sintetizador.falar(resposta)
        except Exception:
            logger.exception("TTS falhou - resposta ficou so na tela")

    bus.subscribe("chat.mensagem_usuario", _ao_receber_chat)
    ...
    sentinela = None  # pode nao existir (visao desabilitada) - o alivio de
    # carga abaixo checa antes de usar.
    ...
    AlivioCarga(
        ...
        pausar_visao=sentinela.pausar if sentinela is not None else None,
        retomar_visao=sentinela.retomar if sentinela is not None else None,
    )
    ...
    if voz is not None:
        ...
        await sintetizador.falar("Oi! Pode escrever comigo pelo chat da tela.")
        logger.info("Pronto! Conversa pelo chat da pagina /controle. Ctrl+C para sair.")

    try:
        if voz is not None:
            await voz.executar()
        else:
            # sem escuta o processo vive dos eventos (chat, conselho,
            # sentinela) - so espera o Ctrl+C.
